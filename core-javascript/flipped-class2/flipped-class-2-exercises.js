...
// */
let primeraPregunta = window.prompt(
  '1. ¿Qué forma tiene el planeta tierra?\nplana\nesferica',
);
let segundaPregunta = window.prompt(
  '2. ¿Cuál es el satélite natural de nuestro planeta?\nLuna\nTitan',
);
if (
  primeraPregunta.toLowerCase() == 'esferica' &&
  segundaPregunta.toLowerCase() == 'luna'
) {
  alert('Muy bien! Has contestado correctamente ambas preguntas');
} else if (
  primeraPregunta.toLowerCase() == 'plana' ||
  segundaPregunta.toLowerCase() == 'titan'
) {
...
let articulos_en_venta = window.prompt(
  '¿De qué producto necesitas saber el precio?\nArroz\nLeche\nHuevos',
);
if (articulos_en_venta.toLowerCase() == 'leche') {
  ...
  let opciones_huevos = window.prompt(
    '¿Qué tipo de huevos?\ncamperos\necologicos',
  );
...
for (
  ...
  for (let multiplicador = 1; multiplicador <= 10; multiplicador++) {
    document.write(
      `${tablas_multiplicar} x ${multiplicador} = ${
        tablas_multiplicar * multiplicador
      } <br>`,
    );
  }
}
...
for (let numero_impar = 1; numero_impar <= numero_entero; numero_impar++) {
  ...
  if (numero_impar + 2 <= numero_entero) {
    document.write(', ');
  } else {
    document.write('.');
  }
...
//EJERCICIO HECHO EN CLASE EL VIERNES 26/10 
const ejercicio = require(`./ejercicio11-2.js`)
